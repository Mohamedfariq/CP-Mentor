...
class ContestScheduleItem(BaseModel):
    """Represents a scheduled contest"""
    contest_id: str
    codeforces_id: str
...
class CreateContestPayload(BaseModel):
    codeforcesId: str
    scheduledAt: int  # Unix timestamp (must be in future)
    durationSeconds: int  # 3600, 7200, or 10800
    topics: list[str] = []  # Topics for contest questions


class UpdateContestPayload(BaseModel):
    contestId: str
    codeforcesId: str
    scheduledAt: int | None = None  # must be in future
    durationSeconds: int | None = None
    topics: list[str] | None = None


class DeleteContestPayload(BaseModel):
    contestId: str
    codeforcesId: str


class GetContestsPayload(BaseModel):
    codeforcesId: str


class StartContestPayload(BaseModel):
    """Start a scheduled contest"""
    contestId: str
    codeforcesId: str


class CompleteContestPayload(BaseModel):
    contestId: str
    codeforcesId: str
    durationSeconds: int = 0
    problemKeys: list[str] = []
...
CLUSTER_MODEL_STRICT = _safe_bool_env("CLUSTER_MODEL_STRICT", False)
CLUSTER_RETRAIN_ENABLED = _safe_bool_env("CLUSTER_RETRAIN_ENABLED", False)
...
ALLOW_CLUSTER_CENTROID_FALLBACK = _safe_bool_env("ALLOW_CLUSTER_CENTROID_FALLBACK", True)
...
def execute_code(payload: ExecuteCodePayload):
    """
    Execute user code against test cases.

    Supports: C++, C, Python, Java, Go, Rust, JavaScript, Kotlin, C#
    """
    if not CodeExecutor:
    ...
    try:
        result = CodeExecutor.execute(
            ...
            test_cases=[
                {
                    "input": tc.input or "",
                    "expectedOutput": tc.expectedOutput or "",
                    "index": tc.index,
                }
...
# ==================== NEW CONTEST MANAGEMENT ENDPOINTS ====================


def _validate_future_timestamp(timestamp: int) -> None:
    """Validate that timestamp is in the future (at least 5 minutes from now)"""
    now_ts = int(time.time())
    min_future_ts = now_ts + 300  # 5 minutes from now
    if timestamp < min_future_ts:
        raise HTTPException(
            status_code=400,
            detail=f"Contest must be scheduled at least 5 minutes in the future. Current time: {now_ts}",
        )
    # Maximum 1 year in future
    max_future_ts = now_ts + (365 * 24 * 60 * 60)
    if timestamp > max_future_ts:
        raise HTTPException(status_code=400, detail="Contest cannot be scheduled more than 1 year in the future")


def _validate_duration(duration_seconds: int) -> None:
    """Validate contest duration is 1, 2, or 3 hours"""
    valid_durations = [3600, 7200, 10800]  # 1h, 2h, 3h in seconds
    if duration_seconds not in valid_durations:
        raise HTTPException(
            status_code=400, detail=f"Duration must be one of: 1 hour (3600s), 2 hours (7200s), or 3 hours (10800s)"
        )


@app.post("/api/contests/create")
def create_contest(payload: CreateContestPayload):
    ...
    _validate_future_timestamp(scheduled_at)
    _validate_duration(payload.durationSeconds)

    topics = [_normalize_topic(t) for t in (payload.topics or [])]
...
@app.post("/api/contests/list")
def list_contests(payload: GetContestsPayload):
    """List all contests for a user"""
    codeforces_id = payload.codeforcesId.strip().lower()
    if not codeforces_id:
        raise HTTPException(status_code=400, detail="codeforcesId is required")
    ...
    try:
        ...
        for contest in contests:
            contest_doc = {
                ...
                "codeforces_id": contest["codeforces_id"],
                "scheduled_at": contest["scheduled_at"],
                "duration_seconds": contest["duration_seconds"],
                "topics": contest.get("topics", []),
                "status": contest["status"],
                "can_start": contest["scheduled_at"] <= now_ts and contest["status"] == "pending",
                "window_end_ts": contest["scheduled_at"] + 3600,  # 1 hour window to start
                "created_at": contest["created_at"],
                "started_at": contest.get("started_at"),
                "completed_at": contest.get("completed_at"),
            }
...
@app.post("/api/contests/update")
def update_contest(payload: UpdateContestPayload):
    """Update a contest schedule (only if not started)"""
    contest_id = payload.contestId.strip()
    codeforces_id = payload.codeforcesId.strip().lower()
    if not contest_id or not codeforces_id:
    ...
    try:
        ...
        # Cannot update if contest has started or completed
        if contest["status"] in ["running", "completed"]:
            raise HTTPException(status_code=400, detail="Cannot update a contest that has started or completed")

        update_data = {}
        if payload.scheduledAt is not None:
            _validate_future_timestamp(payload.scheduledAt)
            update_data["scheduled_at"] = payload.scheduledAt
        ...
        if payload.topics is not None:
            topics = [_normalize_topic(t) for t in payload.topics]
            topics = [t for t in topics if t]
        ...
        return {
            ...
            "duration_seconds": updated_contest["duration_seconds"],
            "topics": updated_contest.get("topics", []),
...
@app.post("/api/contests/delete")
def delete_contest(payload: DeleteContestPayload):
    """Delete a contest"""
    contest_id = payload.contestId.strip()
    codeforces_id = payload.codeforcesId.strip().lower()
    if not contest_id or not codeforces_id:
...
@app.post("/api/contests/start")
def start_contest(payload: StartContestPayload):
    """Start a scheduled contest"""
    contest_id = payload.contestId.strip()
    codeforces_id = payload.codeforcesId.strip().lower()
    if not contest_id or not codeforces_id:
    ...
    try:
        ...
        if contest["status"] != "pending":
            raise HTTPException(status_code=400, detail=f"Contest is already {contest['status']}")

        now_ts = int(time.time())
        scheduled_at = contest["scheduled_at"]
        window_start = scheduled_at
        window_end = scheduled_at + 3600  # 1 hour window after scheduled time

        if now_ts < window_start:
            time_until_start = window_start - now_ts
            raise HTTPException(
                status_code=400,
                detail=f"Contest has not started yet. Starts in {time_until_start} seconds",
            )

        if now_ts > window_end:
            raise HTTPException(
                status_code=400, detail="Contest start window has closed. You can only start within 1 hour of scheduled time."
            )

        # Generate contest problems based on topics
        topics = contest.get("topics", [])
        problems = []

        if not topics:
            raise HTTPException(status_code=400, detail="Contest must have at least one topic")

        # Get problems for each topic
        for topic in topics:
            try:
                topic_problems = _get_problems_for_topic(codeforces_id, topic, limit=1)
